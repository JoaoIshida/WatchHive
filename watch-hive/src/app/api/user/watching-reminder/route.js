...
export async function PATCH(req) {
  try {
    ...
    if (content_id == null || media_type !== 'tv') {
      return new Response(JSON.stringify({ error: 'content_id and media_type tv required' }), {
        status: 400,
      });
    }
    ...
    if (wErr) throw wErr;
    if (!wc) {
      return new Response(JSON.stringify({ error: 'Watched series not found' }), { status: 404 });
    }

    const row = {
      watched_content_id: wc.id,
    ...
    const { error } = await supabase.from('watching_reminders').upsert(row, {
      onConflict: 'watched_content_id',
    });
