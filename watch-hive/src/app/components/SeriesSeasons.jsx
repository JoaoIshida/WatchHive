...
const SeriesSeasons = ({ seriesId, seasons, seriesName = 'Series' }) => {
    ...
    const toggleEpisodeWatched = async (seasonNumber, episodeNumber) => {
        ...
        if (!isWatched) {
            ...
            if (!episode) {
                // Episode doesn't exist in TMDB - it may not have info yet
                setSkippedItems([{
                    type: 'episode',
                    seriesName: seriesName,
                    seasonName: seasonData?.name || `Season ${seasonNumber}`,
                    episodeName: `Episode ${episodeNumber}`,
                    releaseDate: 'Not available in database'
                }]);
    ...
    const toggleSeasonCompleted = async (seasonNumber) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Reload progress to reflect the updated state
                // Force a fresh fetch to ensure we get the latest state
                const progressResponse = await fetch(`/api/series-progress/${seriesId}?t=${Date.now()}`);
                if (progressResponse.ok) {
                    const data = await progressResponse.json();
                    setProgress(data);
                } else {
                    console.error('Error reloading progress after season toggle');
                    // Try to reload again after a short delay
                    setTimeout(async () => {
                        const retryResponse = await fetch(`/api/series-progress/${seriesId}?t=${Date.now()}`);
                        if (retryResponse.ok) {
                ...
                console.error('Error toggling season completion:', errorText);
            ...
            console.error('Error toggling season completion:', error);
        } finally {
            setLoadingSeasons(prev => ({ ...prev, [seasonNumber]: false }));
        }
    };
    ...
    const toggleSeriesCompleted = async () => {
        ...
        const skipped = [];
        if (!isCompleted) {
            // Fetch all regular seasons data (exclude specials)
    ...
    // Separate regular seasons from specials
    const regularSeasons = seasons.filter(season => season.season_number > 0);
        ...
        <div className="futuristic-card p-6">
            ...
            {specials.length > 0 && (
                <div className="mt-8 pt-6 border-t border-charcoal-700">
                    <h3 className="text-xl font-bold text-amber-500 mb-4">Specials</h3>
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                {seasonData.episode_count && (
                                                    <p className="text-sm text-white/70">
                                                        {seasonData.episode_count} {seasonData.episode_count === 1 ? 'episode' : 'episodes'}
                                                    </p>
                                                )}
                                                <div className="mt-2">
                                                    <div className="flex items-center gap-2">
